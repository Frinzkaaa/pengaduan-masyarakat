...
<body class="bg-gradient-to-br from-purple-100 to-pink-100 min-h-screen flex items-center justify-center">
    ...
    <div class="w-full max-w-md bg-white rounded-2xl shadow-lg p-8">
        ...
        <h2 class="text-3xl font-bold text-center text-purple-700 mb-6">Login Masyarakat</h2>
        <form action="{{ route('login') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <div class="relative">
                    <input type="password" name="password" id="masyarakat-password" 
                        class="w-full border rounded-lg pl-10 pr-10 px-3 py-2 focus:ring-2 focus:ring-purple-400 focus:outline-none" 
                        placeholder="Masukkan password" required>
                    <span class="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-400">
                        🔒
                    </span>
                    <span class="absolute inset-y-0 right-0 flex items-center pr-3 cursor-pointer" onclick="togglePassword('masyarakat-password', this)">
                        <svg id="masyarakat-eye" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0zm-9 0a9 9 0 0118 0c0 2.5-2.5 7-9 7s-9-4.5-9-7z" /></svg>
                    </span>
                </div>
            </div>
            <button type="submit" 
        ...
        <div class="text-center mt-4">
            <small>Belum punya akun? <a href="{{ route('register') }}" class="text-purple-500 font-semibold hover:underline">Daftar di sini</a></small>
        </div>
    </div>
